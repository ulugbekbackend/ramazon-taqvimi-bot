...
class DBHelper:
    ...
    def get_calendar_by_region(self,region_id,dt):
        return self.cursor.execute('select * from ramadan_calendar where region_id=? and r_date=?',(region_id,dt)).fetchone()
    
    def time(self,r_date):
        return self.cursor.execute('select * from ramadan_calendar where r_date=?',(r_date,)).fetchone()